...
import { shoes, statistics } from '@/lib/constants'
            ...
            <div className='flex justify-start items-start flex-wrap w-full mt-20 gap-16'>
                {statistics.map((item, index) => (
                    <div key={index}>
                        ...
                        <p className='leading-7 font-montserrat text-slate-400'>{item.label}</p>
                    </div>
                ))}
            </div>
        </div>
        <div className='relative flex-1 flex justify-center items-center xl:min-h-screen max-xl:py-40 bg-purple-200 bg-hero bg-cover bg-center'>
            <Image 
                src={bigShoeImg} 
                alt='shoe collection'
                width={610}
